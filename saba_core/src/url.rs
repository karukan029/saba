...
// #[]：attribute（属性）
// derive属性は、構造体や列挙型に対して自動的にデバッグ表示やクローンメソッド、等価性比較を提供するためのもの
// Debug, Clone, PartialEqトレイトをUrl構造体に自動実装
...
impl Url {
  ...
  fn extract_port(&self) -> String {
    let url_ports: Vec<&str> = self
      .url
      .trim_start_matches("http://")
      .splitn(2, "/")
      .collect();

    if let Some(index) = url_ports[0].find(':') {
      url_ports[0][index + 1..].to_string()
    } else {
      "80".to_string()
    }
  }
...
mod tests {
  ...
  #[test]
  fn test_no_scheme() {
    let url = "example.com:8888/index.html?a=123&b=456".to_string();
